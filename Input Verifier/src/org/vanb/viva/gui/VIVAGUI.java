...
public class VIVAGUI implements ActionListener
{
    ...
    /** 
     * IO stuff. We'll create a connected input & output streams.
     * We'll pass the output stream to VIVA, and that's where it will write its output.
     * We'll read that from the attached input stream in a separate thread, 
     * and then write it to the outputs text box.
     */
    private PipedInputStream is;
    private PipedOutputStream os;
    
    /** 
     * The PrintStream is a convenience. 
     * It'll be wrapped around the OutputStream.
     */
    private PrintStream ps;
    
    /** 
     * This is the thread that will read from the input stream
     * and pass the text along to the outputs text box.
     */
    ...
    public void setLineNumbers()
    {
        ...
        {
            text += (i<10?" ":"") + i + System.getProperty( "line.separator" );
        }
    ...
    /**
     * This listener listens for changes in the Pattern text box.
     * If the user has made changes to the pattern, then some buttons need to
     ...
     * We only need one, might as well create it here.
     */
    private PatternTextDocumentListener listener = new PatternTextDocumentListener();
    
    /**
     * Some text fields are not editable - but still navigable.
     * Java, by default, turns off the cursor (or, Caret) if a text
     * widget isn't editable. We want the cursor. So, we'll have to
    ...
    /**
     * This thread reads VIVA output from the output stream given to VIVA,
     * and writes it to the Outputs Text box.
     * 
     * @author vanb
     */
    ...
    /**
     * Parse the pattern in the pattern text field.
     */
    private void parse()
    {
    ...
    /**
     * Convenience function to write a line of text to the 
     * output window, followed by a blank line for spacing.
     * 
     * @param text Text to write to the output window.
     */
    ...
    public void actionPerformed( ActionEvent ae )
    {
        ...
        if( source==btnLoadPattern )
        {
            ...
            {
                write( "Unable to find Pattern file: " + patternFile.getName() );
            }
            catch( IOException ioe )
            {
                write( "Unable to read Pattern file: " + patternFile.getName() );
            }           
        }
        else if( source==btnSavePattern || source==btnSaveAsPattern )
        {
            ...
            {
                write( "Unable to write Pattern file: " + patternFile.getName() );
            }
